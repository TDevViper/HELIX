...
def test_rebalance_stats():
    ring = ConsistentHashRing(vnodes=100)
    for i in range(3):
        ring.add_node(_make_worker(f"w{i}"))
    stats = ring.rebalance_stats()
    assert stats["physical_nodes"] == 3
    for wid, dist in stats["distribution"].items():
        assert 20 < dist["ring_pct"] < 50  # roughly even
